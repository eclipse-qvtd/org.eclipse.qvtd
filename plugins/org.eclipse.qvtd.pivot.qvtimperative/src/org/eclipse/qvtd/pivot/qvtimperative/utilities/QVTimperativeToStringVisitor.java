...
public class QVTimperativeToStringVisitor extends QVTbaseToStringVisitor implements QVTimperativeVisitor<String>
{
	...
	public @Nullable String visitIfStatement(@NonNull IfStatement asIfStatement) {
		append("if ");
		safeVisit(asIfStatement.getOwnedCondition());
		append(" {");
		for (Statement asStatement : asIfStatement.getOwnedThenStatements()) {
			safeVisit(asStatement);
		}
		append("} else {");
		for (Statement asStatement : asIfStatement.getOwnedElseStatements()) {
			safeVisit(asStatement);
		}
		append("}\n");
		return null;
	...
	@Override
	public String visitImperativeTypedModel(@NonNull ImperativeTypedModel object) {
	...
	@Override
	public @Nullable String visitInitializeStatement(@NonNull InitializeStatement asStatement) {
		if (asStatement.isIsChecked()) {
			append("check ");
		}
		append("init ");
		appendName(asStatement.getTargetVariable());
		append(" := ");
		safeVisit(asStatement.getOwnedInit());
		return null;
	}
