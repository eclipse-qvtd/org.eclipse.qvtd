...
public class TxKeyNodeImpl extends TxNodeImpl implements TxKeyNode {
	...
	public boolean validatePartsAreUnique(final DiagnosticChain diagnostics, final Map<Object, Object> context) {
		...
		if (le) {
			...
			final /*@NonInvalid*/ org.eclipse.ocl.pivot.values.@NonNull CollectionValue BOXED_ownedTxPartNodes = idResolver.createOrderedSetOfAll(UMLXTables.ORD_CLSSid_TxPartNode, ownedTxPartNodes);
			/*@Thrown*/ org.eclipse.ocl.pivot.values.@NonNull MutableIterable accumulator = ValueUtil.createSetAccumulatorValue(UMLXTables.ORD_CLSSid_TxPartNode);
			@NonNull Iterator<Object> ITERATOR__1 = BOXED_ownedTxPartNodes.lazyIterator();
			...
			while (true) {
				...
				if (accumulator.includes(referredEStructuralFeature) == ValueUtil.TRUE_VALUE) {
					...
					accumulator.mutableIncluding(referredEStructuralFeature);
				}
			}
