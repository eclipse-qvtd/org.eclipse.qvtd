...
//			}
	...
	public @NonNull Node visitLetExp(@NonNull LetExp letExp) {
		...
		ClassDatumAnalysis classDatumAnalysis = context.getMappingRegion().getClassDatumAnalysis(ownedVariable);
		...
		if (operationId == standardLibraryHelper.getOclAnyOclAsTypeId()) {
			...
			if (operationNode == null) {
				...
				if (referredOperation.getBodyExpression() != null) {
					...
					QVTm2QVTs qvtm2qvts = (QVTm2QVTs) multiRegion.getScheduleModel();		// FIXME cast
