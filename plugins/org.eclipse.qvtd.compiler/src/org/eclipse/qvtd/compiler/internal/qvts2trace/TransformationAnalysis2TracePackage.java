 ...
 */
public class TransformationAnalysis2TracePackage //extends QVTbaseHelper
{
	 ...
	 */
	protected final @NonNull Map<@NonNull Rule, @NonNull RuleAnalysis2TraceClass> rule2ruleAnalysis2traceClass = new HashMap<>();
	...
	static class OverrideDepthComparator implements Comparator<@NonNull RuleAnalysis2MiddleType>
	{
		public static final @NonNull OverrideDepthComparator INSTANCE = new OverrideDepthComparator();

		@Override
		public int compare(@NonNull RuleAnalysis2MiddleType o1, @NonNull RuleAnalysis2MiddleType o2) {
			int d1 = getDepth(o1);
			int d2 = getDepth(o2);
			if (d1 != d2) {
				return d1 - d2;
		...
		private int getDepth(@NonNull RuleAnalysis2MiddleType relation2middleType) {
			int depth = 0;
			for (Rule aRule = relation2middleType.getRule(); (aRule = aRule.getOverridden()) != null; ) {
				depth++;
			}
			return depth;
		}
	...
	public TransformationAnalysis2TracePackage(@NonNull ScheduleManager scheduleManager, @NonNull TransformationAnalysis transformationAnalysis) {
	...
	public @NonNull RuleAnalysis2TraceClass getRuleAnalysis2TraceClass(@NonNull Rule rule) {
		RuleAnalysis2TraceClass ruleAnalysis2traceClass = rule2ruleAnalysis2traceClass.get(rule);
		if (ruleAnalysis2traceClass == null) {
			...
			if (!QVTbaseUtil.hasOverrides(rule)) {
				ruleAnalysis2traceClass = new NonOverrideRuleAnalysis2TraceClass(ruleAnalysis);
			}
			else if (scheduleManager.isTopLevel(rule)) {
				ruleAnalysis2traceClass = new TopOverrideRuleAnalysis2TraceClass(ruleAnalysis);
			}
			else {
				ruleAnalysis2traceClass = new NonTopOverrideRuleAnalysis2TraceClass(ruleAnalysis);
			}
			rule2ruleAnalysis2traceClass.put(rule, ruleAnalysis2traceClass);
		}
		return ruleAnalysis2traceClass;
	}
	...
	public org.eclipse.ocl.pivot.@NonNull Class getTraceClass(@NonNull Rule rule) {
		return getRuleAnalysis2TraceClass(rule).getMiddleClass();
	}
	...
	public @NonNull VariableDeclaration2TraceProperty getVariableDeclaration2TraceProperty(@NonNull VariableDeclaration variable) {
		RuleAnalysis2TraceClass relation2TraceClass = getRuleAnalysis2TraceClass(QVTbaseUtil.getContainingRule(variable));
		return relation2TraceClass.getVariableDeclaration2TraceProperty(variable);
	}
	 ...
	 * transformation.
	 */
	public org.eclipse.ocl.pivot.@NonNull Package transform(@NonNull TypedModel traceTypedModel) throws CompilerChainException {
		//
		...
		for (@NonNull Rule rule : QVTbaseUtil.getOwnedRules(transformation)) {
			getRuleAnalysis2TraceClass(rule);
		}
		List<@NonNull RuleAnalysis2MiddleType> relation2middleTypes = new ArrayList<>(name2ruleAnalysis2middleType.values());
		...
		//
		for (@NonNull RuleAnalysis2MiddleType relation2middleType : relation2middleTypes) {
