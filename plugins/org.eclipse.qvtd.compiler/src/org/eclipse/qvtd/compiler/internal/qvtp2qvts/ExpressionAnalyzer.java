...
public class ExpressionAnalyzer extends AbstractExtendingQVTimperativeVisitor<@NonNull SimpleNode, @NonNull SimpleMappingRegion>
{
	...
	/**
	 * A ConditionalExpressionAnalyzer is used when analyzing transitively optional computations
	 * such as the then or else arms of an IfExp. It overrides edge and node creations to ensure that
	 * they are marked as UNNAVIGABLE.
	 */
	...
	private @NonNull SimpleNode analyzeOperationCallExp_includes(@NonNull SimpleNode sourceNode, @NonNull OperationCallExp operationCallExp) {
		...
		return createTrueNode();				// Bogus return not actually used
	}
...
//			}
	...
	protected @NonNull SimpleNode createLetNode(@NonNull String name, @NonNull SimpleNode sourceNode) {
		return Nodes.LET.createSimpleNode(context, name, sourceNode);
	}
	...
	protected @NonNull SimpleNode createTrueNode() {
		return Nodes.TRUE.createSimpleNode(context);
	}
