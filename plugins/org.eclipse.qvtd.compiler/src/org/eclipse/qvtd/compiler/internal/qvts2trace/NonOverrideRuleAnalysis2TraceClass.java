 ...
 */
class NonOverrideRuleAnalysis2TraceClass extends AbstractRuleAnalysis2TraceClass
{
	protected NonOverrideRuleAnalysis2TraceClass(@NonNull RuleAnalysis ruleAnalysis) {
	 ...
	 */
	protected boolean needsStatusProperty(@NonNull OCLExpression rInvocation) {
	...
	protected void resolveStatusProperty() {
		if (!rule.isIsAbstract()) {
			Iterable<@NonNull ? extends OCLExpression> incomingInvocations = ruleAnalysis.getIncomingInvocations();
			if (incomingInvocations != null) {
				boolean needsStatusProperty = false;
				for (@NonNull OCLExpression invocation : incomingInvocations) {
