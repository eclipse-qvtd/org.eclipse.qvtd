...
public class PropertyPage extends PreferencePage implements IWorkbenchPreferencePage, IWorkbenchPropertyPage, ModelRegistryHelpIds {
	...
	public static final String PREF_ID = "org.eclipse.qvt.declarative.modelregistry.ui.preferences.ModelRegistryPreferencePage"; //$NON-NLS-1$
	public static final String PROP_ID = "org.eclipse.qvt.declarative.modelregistry.ui.propertyPages.ModelRegistryPreferencePage"; //$NON-NLS-1$
	
	private AbstractFileHandle fileHandle; // resource or null
...
//		setDescription(PreferencesMessages.ModelRegistryPreferencePage_description); 		
	...
	private Combo createAccessorCombo(Composite inner) {
		...
		List<String> kindList = new ArrayList<String>(accessorKindRegistry.getKinds());
		Collections.sort(kindList);
		String[] kindArray = kindList.toArray(new String[kindList.size()]);
		Label accessorKindLabel = new Label(inner, SWT.NONE);
		accessorKindLabel.setText(PreferencesMessages.ModelRegistryConfigurationBlock_accessor_label); 
		accessorCombo = new Combo(inner, SWT.NONE);
		accessorCombo.setItems(kindArray);
		accessorCombo.select(accessorCombo.indexOf(ModelNameAccessor.KIND));
		accessorCombo.addSelectionListener(new SelectionAdapter() {
			@Override public void widgetSelected(SelectionEvent e) {
				if (!configurationBlock.setAccessorKind(accessorCombo.getText()))
					accessorCombo.select(accessorCombo.indexOf(configurationBlock.getAccessorKind()));
			}
		});
		ModelRegistryHelper.setHelp(accessorKindLabel, ModelRegistryHelpIds.General.ACCESSOR_KIND);
...
//		}
	...
	protected AbstractProjectHandle getProjectHandle() {
