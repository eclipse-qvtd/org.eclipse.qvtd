...
/*	private static class RegistrationSorter extends ViewerComparator
	...
	protected final AbstractFileHandle fileHandle;		// File handle of selected resource
...
//		}
	...
	public String getAccessorKind() { return accessorKind; }
	...
	private AccessorRegistry<?> getAccessorRegistry() {
		...
		Class<? extends Accessor> accessorClass = accessorKindRegistry.getAccessorClass(accessorKind);
...
//		int candidateDepth = candidateRegistration.getFileHandleRegistry().getDepth();
	...
	private void loadEditModel() {
		...
		Class<? extends Accessor> accessorClass = accessorKindRegistry.getAccessorClass(accessorKind);
		if (accessorClass != null) {
			for (AbstractFileHandle fileHandleAncestor = fileHandle; fileHandleAncestor != null; fileHandleAncestor = fileHandleAncestor.getParentFileHandle()) {
	...
	public boolean setAccessorKind(String accessorKind) {
		if (!this.accessorKind.equals(accessorKind)) {
			if (isDirty()) {
				if (!MessageDialog.openConfirm(shell, "Save Changes?", "The '" + accessorKind + "' accessor has unsaved changes. Do you wish to save them?"))
					return false;
				saveEditModel();
			}
